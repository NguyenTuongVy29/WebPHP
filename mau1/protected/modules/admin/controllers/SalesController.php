...
class SalesController extends Controller
{
	...
	public function actionAddCart(){
            ...
            foreach($data as $item){
                  $i++;
                  $message.= '<tr><td>'.$i.'</td><td>'.$item['name'].'</td><td><img src="http://localhost:8080/bkap/'.$item['image'].'" width=100 /></td><td>'.$item['price_new'].'</td><td>'.'<button type="button" class="btn btn-success" onclick="addBill('.$item['id_product'].')">Mua Hàng</button>'.'</td></tr>';
            }
             $message.='</table>';
             echo $message;
        }
    ...
    public function  actionViewBill(){
        ...
        $message='<div class="xxx1">        <h4>KUTESHOP</h4>    </div>';
        ...
        $message.='<tr><td colspan="3">Tiền Trả lại</td><td><span id="money_tra"></span></td></tr>';
        $message.='</table>'; $message.='</br>';
         $message.='<p style="text-align: left;font-size: 9px">Công ty THHH KUTESHOP</p>';
